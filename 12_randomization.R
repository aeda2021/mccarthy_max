...
# These two distributions are generally similar in shape - both somewhat left-skewed.
...
# The two distirubitons don't quite have the same mean (slightly higher in the resampled data)

# Q: what would happen if you sampled without replacement?

# If sampling were done without replacement, the two distributions would have the exact same mean.
...
pennies_resample_means <- pennies_resample %>% group_by(replicate) %>% summarise(mean_year = mean(year))
hist(pennies_resample_means$mean_year, breaks = 30)
...
# Resampling a single sample with replacement could result in occasional samples with relatively large numbers of high or low values, allowing for a large spread in the distribution of means.
...
# I predict that as the number of reps decreases, the distribution will less closely approximate a normal distribution.

pennies_resample2 <- pennies_sample %>% 
  rep_sample_n(size = 50, replace = TRUE, reps = 50)
resample_means2 <- pennies_resample2 %>% group_by(replicate) %>% summarise(ymean = mean(year))
hist(resample_means2$ymean, breaks = 30)
...
pennies_resample3 <- pennies_sample %>% 
  rep_sample_n(size = 20, replace = TRUE, reps = 50)
resample_means3 <- pennies_resample3 %>% group_by(replicate) %>% summarise(ymean = mean(year))
hist(resample_means3$ymean, breaks = 30)

# now let's get the bootstrapped CI
CI <- pennies_resample_means %>%
  summarize(
    l = quantile(mean_year, prob = 0.025), 
    u = quantile(mean_year, prob = 0.975)
  )
...
ggplot(data = pennies_resample_means, aes(x=mean_year)) +
...
pennies_resample_medians <- pennies_resample %>% group_by(replicate) %>% summarise(median_year = median(year))
hist(pennies_resample_medians$median_year, breaks = 30)

CI_median <- pennies_resample_medians %>%
  summarize(
    l = quantile(median_year, prob = 0.025), 
    u = quantile(median_year, prob = 0.975)
  )

ggplot(data = pennies_resample_medians, aes(x=median_year)) +
  geom_histogram(binwidth = 1, color = "white") +
  geom_vline(xintercept = CI_median$l, color = "green") +
  geom_vline(xintercept = CI_median$u, color = "green")
...
# These data aren't tidy (columns include information on both year and counts of bees), but this should be fine if we
# are looking to shuffle counts of bees among plants within each year.
...
# We're sampling the same number of values, but without replacement, which will simply result in randomizing the order of
# the these values (permuting them).
...
# Column sums are maintained - this should be the case, since we're interested in randomizing among plants within years.

rowSums(bee_prefs)
rowSums(bee_prefs_perm)
# Q: are row sums maintained?  should they be?

# Row sums aren't maintained because we've randomized numbers of visits among plants within each year,
# so it's expected that some plants will no longer have received the same total number of visits across years as before.

# Q: write code below to do a permutation similar to that above,
# but now permute the rows instead of the columns

bee_years_perm = matrix(0,15,4)
for (i in 1:nrow(bee_years_perm)) {
  bee_years_perm[i,] = sample(bee_prefs[i,], replace=F)
}
...
# This functions simulates pollen deposition over 10000 site-dates by randomly drawing from potential numbers of visits
# per site date and from potential amounts of pollen deposited per visit.

# Q: why did we use size=visit_draw?

# We used size = visit_draw because we need to sample an amount of pollen deposited for each visit we're simulating.
...
hist(rgamma(173, shape = shape, scale = scale), breaks = 50)
hist(visits$visits, breaks = 50)
...
# gamma still seems to fit best and has the lowest AIC

# Q: write code below to choose a distribution for the pollen data

hist(pollen$consp_grains, breaks = 50)
mean_pol1 <- mean(pollen$consp_grains)
mean_pol1
hist(rpois(173, mean_pol1), breaks = 50)

sd_pol1 <- sd(pollen$consp_grains)
sd_pol1
shape1 <- (mean_pol1/sd_pol1)^2
shape1
scale1 <- sd_pol1^2 / mean_pol1
scale1

hist(rgamma(173, shape = shape1, scale = scale1), breaks = 50)

# A gamma distribution fits these data much more closely than Poisson.
...
# It returned means of 100 bootstrapped samples from the dataset (sampled with replacement).
...
# The quantities in the stat column are differences in mean mass of queens and workers in each permuted sample.

# Q: how will you use these numbers? what are we trying to test here?

# We're trying to test whether there is a difference in masses of queens and workers - these mean differences from
# permutations give differences in mass between queens and workers that could be expected to occur even if the 
# masses of these two groups don't actually differ significantly.
...
# This is the mean difference in mass between queens and workers if there is in fact no difference in their masses.
# This number is quite close to zero, which is expected if the masses of these two groups do not actually differ.
...
# I expect that this permutation test will conclude that masses of queens and workers differ significantly - the actual 
# difference in the masses of these two groups is far larger than what might be expected to occur by chance if the groups
# don't actually differ.
...
# The bootstrapped results may not be particularly meaningful considering that the distribution of masses of bees may
# actually be bimodal.
...
nrow(filter(null_dist, stat >= obs_stat))/nrow(null_dist)
