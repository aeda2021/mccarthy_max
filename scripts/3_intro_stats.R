...
predicted_car_mod <- augment(car_mod, interval = 'prediction')
ggplot(predicted_car_mod, aes(x = wt, y = mpg)) + 
  geom_point() + 
  geom_line(aes(y = .fitted)) +  # augment uses .whatever for all the new columns it creates to avoid confusion with existing variables
...
ggplot(iris, aes(x = Species, y = Sepal.Width)) + geom_boxplot(alpha = 0.1)
...
iris %>% group_by(Species) %>% summarize(mean(Sepal.Width))
...
# Residuals vs. Fitted: scatter of points doesn't seem to be particularly even around zero; more negative residuals for intermediate fitted values
# Normal Q-Q plot: perhaps some right skew? Otherwise doesn't seem to deviate too greatly from normal.
# Scale-Location: variance seems relatively even across the range of fitted values
# Residuals vs. Leverage: no points with Cook's D < 1 (none > 0.5, in fact); a few points have large residuals, but not too much leverage, while one has high leverage but a relatively small residual
...
# Both weight and horsepower are significant predictors of the mileage of cars in this data. Holding horsepower constant, increasing weight by 1 ton decreases car mileage by approximately 3.88 miles per gallon; at a constant weight, an increase of 100 horsepower decreases mileage by approximately 3.18 miles per gallon.
...
ggplot(limpet, aes(x = DENSITY, y = EGGS, color = SEASON)) +
        geom_point() +
        scale_color_manual(values = c(spring="darkgreen", summer="orange")) +  # this is how you set colors manually in ggplot, if things like this are important to you
...
# Prediction: mean number of eggs varies by both season and density; negative effect of density, significantly higher mean number of eggs laid in spring vs. summer.
...
ggplot(limpet, aes(x = DENSITY, y = EGGS, color = SEASON)) +
        geom_point() +
        scale_color_manual(values = c(spring="darkgreen", summer="orange")) +  
...
# Doesn't seem to be an interaction - trends with density (slopes) seem similar between spring and summer.
...
# Residuals vs. Fitted: Equal variances across values of x? Appears so; reasonably even scatter of points around 0
# Normal Q-Q: Normality - are standardized residuals normally distributed? Arrangement of points is fairly linear, so likely that residuals are approximately normally distributed, although this includes points for both spring and summer groups.
# Scale-Location: Is variance approximately equal across all values of x?
# Constant Leverage: Are there any overly-influential data points?
...
ggplot(limpet, aes(x = DENSITY, y = EGGS, color = SEASON)) +
        geom_point() +
        scale_color_manual(values = c(spring="darkgreen", summer="red")) +  
		geom_abline(intercept = coef(limpet_mod)[1], slope = coef(limpet_mod)[2], colour="darkgreen") +
...
ggplot(mtcars, aes(x = hp_standardized, y = mpg)) + geom_point() + geom_abline(intercept = coef(car_mod5)[1], slope = coef(car_mod5)[2]) + geom_vline(xintercept = 0, linetype="dashed")
