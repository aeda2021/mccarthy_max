...
# A: It isn't normal because counts are bounded at zero (and the mean of this distribution is quite low, so it will not even be approximately normal).
...
hist(rbinom(500, 100, 0.2))
hist(rbinom(500, 1000, 0.2))


# Q: what is plotted on the x axis exactly, in a histogram from rbinom?
# A: The x axis plots the number of successes in a given number of trials.
...
colnames(soay)
ggplot(data = soay, aes(x = body.size, y = fitness)) + geom_point() + geom_smooth(span = 1, color = "red", se = FALSE)
...
soay.lm <- lm(fitness~body.size, data = soay)
summary(soay.lm)
hist(soay.lm$residuals)
autoplot(soay.lm)

# Q: do these data meet lm assumptions?
# A: No - residuals don't seem to be normally distributed and also seem to violate homoskedasticity (there seems to be a positive relationship between mean and variance, as demonstrated by the Scale-location plot).
...
# These are count data (bounded at zero) with a low mean value.
...
# A: The glm has a lower AIC because the link function and poisson error allow it to better explain the variability in the data than the lm (using the same number of predictors).
...
#A: This compares survival between male and female passengers (as represented by the numbers of points in each quadrant).
...
# A: This groups the data by sex and calculates the proportion of individuals who survived.
